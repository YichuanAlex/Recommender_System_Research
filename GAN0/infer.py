...
def infer():
    ...
    # 初始化GANQuantizerWrapper和semantic_emb
    mm_emb_id = args.mm_emb_id[0]
    mm_emb_dim = 32 if mm_emb_id == '81' else 1024  # 根据实际情况调整
    num_classes = 256  # 根据GAN量化器类别数调整
    ...
    semantic_emb = torch.nn.Embedding(num_classes, args.hidden_units).to(args.device)
...
if __name__ == "__main__":
    ...
    print("Inference completed. Top-10 results for each user:")
    for user, top10 in zip(user_list, top10s):
        print(f"User: {user}, Top-10 items: {top10}")