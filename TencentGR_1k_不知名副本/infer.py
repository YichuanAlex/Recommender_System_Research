...
def get_args():
    ...
    # MMemb Feature ID
    parser.add_argument('--mm_emb_id', nargs='+', default=['81','82','83','84','85','86'], type=str, choices=[str(s) for s in range(81, 87)])
...
def read_result_ids(file_path):
    with open(file_path, 'rb') as f:
        ...
        if num_points_query == 0:
            return np.array([]).reshape((0, query_ann_top_k))
...
def get_candidate_emb(indexer, feat_types, feat_default_value, mm_emb_dict, model):
    ...
    candidate_path = Path(os.environ.get('EVAL_DATA_PATH'), 'predict_seq.jsonl')
    item_ids, creative_ids, retrieval_ids, features = [], [], [], []
    retrieve_id2creative_id = {}

    # Keep track of items we've already processed to avoid duplicates
    processed_items = set()
    
    with open(candidate_path, 'r') as f:
        for line in f:
            user_interactions = json.loads(line)
            # Process each interaction in the user's sequence
            for interaction in user_interactions:
                _, item_id, _, feature, _, _ = interaction
                # For creative_id and retrieval_id, we'll use item_id as a placeholder
                # since they're not directly available in the interaction data
                creative_id = item_id  # Placeholder, adjust if you have a mapping
                retrieval_id = item_id  # Placeholder, adjust if you have a mapping
                ...
                item_index = indexer.get(creative_id, 0)  # Use .get() to avoid KeyError
                missing_fields = set(
...
def infer():
    ...
    with open(candidate_emb_path, 'rb') as f:
        ...
        # 读取嵌入向量
        candidate_embs = np.fromfile(f, dtype=np.float32).reshape(-1, num_dimensions)
    
    # 创建FAISS索引
    dimension = all_embs.shape[1]
    index = faiss.IndexHNSWFlat(dimension, 64)  # 使用HNSW索引，M=64
    index.hnsw.efConstruction = 1280  # 设置efConstruction参数
    
    # 添加候选库的embedding到索引中
    index.add(candidate_embs)
    
    # 设置检索参数
    index.hnsw.efSearch = 640  # 设置efSearch参数
    
    # 执行检索
    top_k = 10
    distances, indices = index.search(all_embs, top_k)
    
    # 保存检索结果
    result_id_file_path = Path(os.environ.get("EVAL_RESULT_PATH"), "id100.u64bin")
    with open(result_id_file_path, 'wb') as f:
        # 写入头部信息
        num_points_query = all_embs.shape[0]
        f.write(struct.pack('II', num_points_query, top_k))
        # 写入检索结果
        indices.astype(np.uint64).tofile(f)
