...
class MyDataset(torch.utils.data.Dataset):
    ...
    def _load_user_data(self, uid):
        ...
        # 检查是否为空行
        if not line.strip():
            print(f"Warning: Empty line found at offset {self.seq_offsets[uid]}, skipping.")
            return []
        try:
            data = json.loads(line)
            return data
        except json.JSONDecodeError as e:
            raise ValueError(f"Invalid JSON at offset {self.seq_offsets[uid]}: {e}")
    ...
    def _init_feat_info(self):
        ...
        for feat_id in feat_types['item_emb']:
            # Check if mm_emb_dict[feat_id] is empty
            if not self.mm_emb_dict[feat_id]:
                print(f"Warning: mm_emb_dict for feat_id {feat_id} is empty. Using a default zero vector of size 32.")
                # Use a default shape, e.g., 32, or handle this case as appropriate for your application
...
class MyTestDataset(MyDataset):
        ...
        for feat_id, feat_value in feat.items():
            if type(feat_value) == list:
                ...
                if feat_id in self.feature_types['item_emb']:
                    emb_dim = self.ITEM_EMB_FEAT.get(feat_id, 32)  # 默认32维
                    processed_feat[feat_id] = np.zeros(emb_dim, dtype=np.float32)
                else:
    ...
    def __getitem__(self, uid):
        ...
        ext_user_sequence = []
        user_id = None  # Initialize user_id to prevent UnboundLocalError
        for record_tuple in user_sequence:
    ...
    for feat_id in tqdm(feat_ids, desc='Loading mm_emb'):
        ...
        if feat_id != '81':
            try:
                ...
                json_files = list(base_path.glob('part-*'))  # Match files without .json extension
                    ...
                    with open(json_file, 'r', encoding='utf-8') as file:
                        for line_num, line in enumerate(file, 1):
                            ...
                            try:
                                ...
                                if 'anonymous_cid' not in data_dict_origin:
                                    print(f"Warning: Skipping line {line_num} in {json_file} due to missing 'anonymous_cid' key")
                                    continue
                                # Handle missing 'emb' key by using a default zero vector
                                if 'emb' not in data_dict_origin:
                                    print(f"Warning: Missing 'emb' key in line {line_num} of {json_file}, using zero vector")
                                ...
                                print(f"Warning: Skipping line {line_num} in {json_file} due to JSON decode error: {e}")
                                continue
        ...
        if feat_id == '81':
            try:
                ...
                # If .pkl file is not found, try to load from .json files like other feature IDs
                try:
                    base_path = Path(mm_path, f'emb_{feat_id}_{shape}')
                    json_files = list(base_path.glob('part-*'))  # Match files without .json extension
                    for json_file in json_files:
                    ...
                    print(f"Error loading emb_{feat_id}_{shape} from json files: {e}")
                    # Re-raise the exception to stop execution
                    raise e
            # Check if emb_dict is empty after attempting to load from JSON files
            if not emb_dict:
                print(f"Warning: No embeddings loaded for feat_id {feat_id}. The emb_dict is empty.")
                # Depending on requirements, you might want to raise an exception or continue with an empty dict
                # For now, we'll continue with an empty dict, but this might cause issues later
                # raise ValueError(f"No embeddings loaded for feat_id {feat_id}")
