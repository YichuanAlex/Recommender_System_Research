...
def get_candidate_emb(indexer, feat_types, feat_default_value, mm_emb_dict, model, dataset):
    ...
    # 保存候选库的embedding和sid（注意：save_item_emb 需要 dataset 用于特征张量化）
    model.save_item_emb(item_ids, retrieval_ids, features, os.environ.get('EVAL_RESULT_PATH'), dataset)
...
def infer():
    ...
    test_loader = DataLoader(
        ...
        pin_memory=True if str(args.device).startswith('cuda') else False,
    )
    usernum, itemnum = test_dataset.usernum, test_dataset.itemnum
    feat_statistics, feat_types = test_dataset.feat_statistics, test_dataset.feature_types

    # 先在 CPU 上加载权重，避免 GPU 反序列化瞬时占用导致 OOM
    ...
    try:
        ...
        if 'CUDA out of memory' in str(e):
            print('[infer] CUDA OOM when moving model to GPU, falling back to CPU inference')
            args.device = 'cpu'
            model = model.to('cpu')
        else:
            raise
    ...
    # 按顺序推理（DataLoader shuffle=False），与样本顺序一致
    with torch.no_grad():
        for step, batch in tqdm(enumerate(test_loader), total=len(test_loader)):
            # 解包 batch（与 MyDataset.collate_fn 对齐）
            seq, pos, neg, token_type, next_token_type, next_action_type, seq_feat, pos_feat, neg_feat = batch
            non_blocking = True if str(args.device).startswith('cuda') else False
            ...
            if str(args.device).startswith('cuda'):
                with torch.autocast(device_type='cuda', dtype=torch.float16):
                    logits = model.predict(seq, seq_feat, token_type, test_dataset)
            else:
                logits = model.predict(seq, seq_feat, token_type, test_dataset)
            # 收集 batch 的所有用户 embedding
            batch_emb = logits.detach().cpu().numpy().astype(np.float32)
    ...
    # 根据样本顺序生成 user_list（与 DataLoader 顺序一致）
    user_list = [test_dataset.indexer_u_rev[i] for i in range(len(test_dataset))]
