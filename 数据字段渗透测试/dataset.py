...
class MyDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_dir, args):
        ...
        self.mm_emb_dict = load_mm_emb(Path(data_dir, "creative_emb"), self.mm_emb_ids, self.indexer)
    ...
    def _init_feat_info(self):
        ...
        for feat_id in feat_types['item_emb']:
            if self.mm_emb_dict[feat_id] and len(self.mm_emb_dict[feat_id]) > 0:
                ...
                feat_default_value[feat_id] = np.zeros(SHAPE_DICT[feat_id], dtype=np.float32)
...
def load_mm_emb(mm_path, feat_ids, indexer=None):
    """
    加载多模态特征Embedding
    ...
    Args:
        ...
        indexer: 索引字典，用于将anonymous_cid映射到re-id后的item_id
    ...
    for feat_id in tqdm(feat_ids, desc='Loading mm_emb'):
                ...
                for json_file in base_path.glob('*.json'):
                    with open(json_file, 'r', encoding='utf-8') as file:
                        for line in file:
                            ...
                            # 将anonymous_cid映射到re-id后的item_id
                            anonymous_cid = data_dict_origin['anonymous_cid']
                            if indexer and str(anonymous_cid) in indexer['i']:
                                item_id = indexer['i'][str(anonymous_cid)]
                                data_dict = {item_id: insert_emb}
                                emb_dict.update(data_dict)
                            else:
                                # 如果没有提供indexer或映射失败，则直接使用anonymous_cid
        ...
        print(f'Loaded #{feat_id} mm_emb, items: {len(emb_dict)}')
    return mm_emb_dict
