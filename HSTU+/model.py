...
class HSTU(torch.nn.Module):
    ...
    def __init__(self, hidden_units, num_heads, dropout_rate, maxlen):
        ...
        self.hierarchical_norm = torch.nn.LayerNorm(hidden_units, eps=1e-8)
        self.hierarchical_linear = torch.nn.Linear(hidden_units, hidden_units)
        
        # Sparse attention components
        self.sparse_attention = FlashMultiHeadAttention(hidden_units, num_heads, dropout_rate)
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        hierarchical_output = self.hierarchical_norm(attn_output)
        hierarchical_output = F.relu(self.hierarchical_linear(hierarchical_output))
        
        # Combine sparse attention and hierarchical outputs
        output = attn_output + hierarchical_output
...
class BaselineModel(torch.nn.Module):
    ...
    def __init__(self, user_num, item_num, feat_statistics, feat_types, args):  #
        ...
        for _ in range(args.num_blocks):
            ...
            # 使用HSTU模块替换原有的FlashMultiHeadAttention
            new_attn_layer = HSTU(
                args.hidden_units, args.num_heads, args.dropout_rate, args.maxlen
            )
