...
def print_mm_emb_info(dataset):
    ...
    #dataset.item_feat_dict
    print(f"曹可心需要的dataset.item_feat_dict: {len(dataset.item_feat_dict.keys())}")
    print(f"曹可心需要的dataset.item_feat_dict: {len(dataset.item_feat_dict.keys())}")
    print(f"曹可心需要的dataset.item_feat_dict: {len(dataset.item_feat_dict.keys())}")
    print(f"曹可心需要的dataset.item_feat_dict: {len(dataset.item_feat_dict.keys())}")
    print(f"曹可心需要的dataset.item_feat_dict: {len(dataset.item_feat_dict.keys())}")
    ...
    # 添加emb81和emb82集合关系分析
    # 获取emb81和emb82的物品集合
    emb81_items = set(dataset.mm_emb_dict.get('81', {}).keys())
    emb82_items = set(dataset.mm_emb_dict.get('82', {}).keys())
    total_items = set(dataset.indexer_i_rev.keys())
    
    print("\n==================== emb81和emb82集合关系分析 ====================")
    print(f"emb81物品数量: {len(emb81_items)}")
    print(f"emb82物品数量: {len(emb82_items)}")
    print(f"总物品数量: {len(total_items)}")
    
    # 基于容斥原理计算7个部分
    # 1. 只在emb81中存在
    only_in_emb81 = emb81_items - emb82_items
    # 2. 只在emb82中存在
    only_in_emb82 = emb82_items - emb81_items
    # 3. 在emb81和emb82中都存在
    in_both = emb81_items & emb82_items
    # 4. 在emb81中存在，但不在总物品中
    emb81_not_in_total = emb81_items - total_items
    # 5. 在emb82中存在，但不在总物品中
    emb82_not_in_total = emb82_items - total_items
    # 6. 在总物品中存在，但不在emb81和emb82中
    in_total_not_in_either = total_items - (emb81_items | emb82_items)
    # 7. 在三个集合中都存在
    in_all_three = emb81_items & emb82_items & total_items
    
    print("\n基于容斥原理的7个部分分析:")
    print(f"  1. 只在emb81中存在: {len(only_in_emb81)} 个物品")
    print(f"  2. 只在emb82中存在: {len(only_in_emb82)} 个物品")
    print(f"  3. 在emb81和emb82中都存在: {len(in_both)} 个物品")
    print(f"  4. 在emb81中存在但不在总物品中: {len(emb81_not_in_total)} 个物品")
    print(f"  5. 在emb82中存在但不在总物品中: {len(emb82_not_in_total)} 个物品")
    print(f"  6. 在总物品中存在但不在emb81和emb82中: {len(in_total_not_in_either)} 个物品")
    print(f"  7. 在三个集合中都存在: {len(in_all_three)} 个物品")
    
    # 打印每个部分的特征维度信息
    print("\n各部分物品的特征维度信息:")
    print(f"  emb81特征维度: {SHAPE_DICT.get('81', 'N/A')} (32)")
    print(f"  emb82特征维度: {SHAPE_DICT.get('82', 'N/A')} (1024)")
